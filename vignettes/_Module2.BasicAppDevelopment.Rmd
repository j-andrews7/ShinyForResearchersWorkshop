...
For instance, if we're designing a Shiny app to explore the a given dataset, we might want to allow users to:
...
and the server function contains a big if-else statement that will become even more unwieldy if we decide to add more plot types in the future.
...
If we had thought about the possibility of multiple plot types from the start,
we could have designed the UI and server in a way that more easily accommodates this feature.
